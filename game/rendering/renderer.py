...
_EQUIPMENT_SLOT_LABELS = {"head": "Head", "body": "Body"}
...
class Renderer:
    ...
    def draw(
        ...
        *, settings_open: bool = False, character_select_open: bool = False,
        furnace_manager=None, notifications=None,
    ) -> None:
    ...
    def _draw_equipment_column(self, window, player) -> None:
        ...
        if selected is not None:
            ...
            if selected_def.is_weapon:
                attack_text = f"{selected_def.damage:.0f}"
        stats_lines = [
            f"HP: {player.health:.0f}/{player.max_health}",
            f"Defense: {player.equipment.total_defense():.0f}",
