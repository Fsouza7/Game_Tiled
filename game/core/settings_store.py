"""Persisted user preferences: currently just music/SFX volume, more
could join later (key bindings, etc.). Deliberately separate from
save_system.py's save slot -- preferences should survive even with no
save yet, aren't part of "one playthrough" (Restart/Load rebuild the run
but must never reset how loud the player set things), and load once at
GameApp startup rather than per-run.
...
preferences are not worth ever blocking game startup over, the same
...
class Settings:
    def __init__(self, music_volume: float = MUSIC_VOLUME, sfx_volume: float = SFX_VOLUME):
...
def save(settings: Settings, path: str = SETTINGS_FILE_PATH) -> None:
    try:
        ...
        with open(path, "w") as f:
            json.dump({"music_volume": settings.music_volume, "sfx_volume": settings.sfx_volume}, f)
    except Exception:
